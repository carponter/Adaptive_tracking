...
class CQLSAC_CNN_LSTM_with_CVAE(nn.Module):
    ...
    def learn(self, experiences):
        """
        从经验中学习，完全重写父类方法以确保任务嵌入的正确处理
        简化版本：训练时必须直接提供任务嵌入
        
        Args:
            experiences: 必须是包含6个元素的元组，第6个元素是任务嵌入
                (states, actions, rewards, next_states, dones, task_embeddings)
        """
        if len(experiences) != 6:
            raise ValueError("训练时必须提供包含任务嵌入的6元素experiences元组")
            
        states, actions, rewards, next_states, dones, task_embeddings = experiences
        
        if not isinstance(task_embeddings, torch.Tensor):
            raise ValueError(f"任务嵌入必须是torch.Tensor类型，而不是 {type(task_embeddings)}")
            
        print(f"使用提供的任务嵌入，形状为 {task_embeddings.shape}")
